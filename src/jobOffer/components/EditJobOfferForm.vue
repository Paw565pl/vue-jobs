...
import useUpdateJobOffer from "@/jobOffer/composables/useUpdateJobOffer"
...
const initialValues = jobOffer.value as JobOfferFormValues | undefined
const { mutate: updateJobOffer } = useUpdateJobOffer(jobOfferId.toString())
...
const { handleSubmit } = useForm<JobOfferFormValues>({ validationSchema, initialValues })

const onSubmit = handleSubmit((values) =>
  updateJobOffer(values, {
    onSuccess: (updatedJobOffer) => {
      toast.success("Job Offer updated successfully!")
      router.push(`/job-offers/${updatedJobOffer.id}`)
    },
    onError: (error) => {
      toast.error("Job Offer was not updated!")
      console.error(error)
    }
  })
)
  ...
  <section class="bg-green-50">
    <div class="container m-auto max-w-2xl py-14">
      ...
      <div v-else class="m-4 mb-4 rounded-md border bg-white px-6 py-8 shadow-md md:m-0">
        <h2 class="mb-6 text-center text-3xl font-semibold">Edit Job Offer</h2>
